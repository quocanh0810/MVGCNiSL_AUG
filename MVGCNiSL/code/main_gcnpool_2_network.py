...
def init_argparse():
    ...
    # Thay đổi cách nhận tham số của indep_graph
    parser.add_argument('--indep_graph', type=lambda s:[item.strip() for item in s.split(",") if item != ""], 
                        default=['co-ess'], 
                        # ['PPI-genetic','PPI-physical','co-exp','co-ess']
                        help="Comma-separated list of cell-independent graphs to use, e.g., 'PPI-genetic,PPI-physical'")

    parser.add_argument('--node_feats', type=str, default="precomputed", help="gene node features")
    ...
    parser.add_argument('--epochs', type=int, default=3000, help="number of maximum training epochs")
    parser.add_argument('--batch_size', type=int, default=512, help="batch size")
    parser.add_argument('--out_channels', type=int, default=64, help="dimension of output channels")
    parser.add_argument('--patience', type=int, default=350, help="patience in early stopping")
...
if __name__ == "__main__":
    ...
    # print("Number of input graphs: {}".format(len(graph_input)))
     ...
     # Chọn chỉ một mạng để chạy, ví dụ như PPI-physical
    graph_input = ['SL','co-ess']
    # ['PPI-genetic','PPI-physical','co-exp','co-ess']
    
    print("Running with input graph:", graph_input)
    print("Number of input graphs: {}".format(len(graph_input)))
    ...
    # Tải lại trọng số của mô hình đã lưu nếu có
    try:
        model.load_state_dict(torch.load('./model_weights/model_weights.pth'))
        model.eval()  # Đặt mô hình ở chế độ đánh giá
        print("Model weights have been loaded from model_weights.pth")
    except FileNotFoundError:
        print("No pre-trained weights found. Starting with a new model.")
    ...
    # Tạo tên file động với thời gian hiện tại
    current_time = datetime.now().strftime('%Y%m%d_%H%M%S')
    file_name = f'model_weights_{current_time}.pth'

    # Lưu lại trọng số của mô hình vào thư mục model_weights với tên file động
    torch.save(model.state_dict(), f'model_weights/{file_name}')
    print(f"Model weights have been saved to model_weights/{file_name}")
    ...
    if args.save_results:
        ...
        file_name = "../results/MVGCN_{}_{}_{}_{}.json".format(args.data_source, args.split_method, str(random_key), current_date)
