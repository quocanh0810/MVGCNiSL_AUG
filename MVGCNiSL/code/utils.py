...
def load_SL_data(cell_name, threshold=-3):
    if cell_name != "synlethdb":
        ...
        # In ra danh sách gene
        # print("Danh sách all_genes:", all_genes)
        print("Tổng số gene:", len(all_genes))
    else:
...
def augment_embeddings(precomputed_embeddings, aug_ratio):
    kgemb_data_mean = np.mean(precomputed_embeddings, axis=0)
    
    augmented_embeddings = copy.deepcopy(precomputed_embeddings)
    
    for i in range(precomputed_embeddings.shape[0]):
        gene_feat = precomputed_embeddings[i]
        
        id_lst = list(range(gene_feat.shape[0]))
        random.shuffle(id_lst)
        
        mask_ids = id_lst[:int(len(id_lst) * aug_ratio)]
        
        gene_feat_aug = copy.deepcopy(gene_feat)
        
        for id in mask_ids:
            gene_feat_aug[id] = kgemb_data_mean[id]
        
        augmented_embeddings[i] = gene_feat_aug
...
def choose_node_attribute(attr, gene_mapping, cell_name, graph_data):
    ...
    elif attr == 'precomputed':
        # Load danh sách gene và embedding từ file mới
        #Jurkat
        # gene_list_407 = pd.read_csv("../data/TransEdata/Jurkat/common_genes_Jurkat_MVGCNiSL_embeding.csv")  #407 gene trùng
        # precomputed_embeddings = np.load("../data/TransEdata/Jurkat/common_genes_embedding_128_sorted.npy")  #embedding transE của 407 gene từ bài cũ

        #K562
        gene_list_407 = pd.read_csv("../data/TransEdata/kg_embed/common_genes_K562_MVGCNiSL_embeding.csv")
        precomputed_embeddings = np.load("../data/TransEdata/kg_embed/embeding_sort/common_genes_embedding_128_sorted.npy")

        # Kiểm tra kích thước của embedding và số gene
        assert precomputed_embeddings.shape[0] == len(gene_list_407), "Số lượng gene không khớp với số lượng embedding"

        # Lọc các gene có trong gene_mapping và file 407 gene mới
        valid_genes = [gene for gene in gene_mapping.keys() if gene in gene_list_407['unique_genes'].values]

        # Tạo một ma trận embedding mới chỉ với các gene hợp lệ
        filtered_embeddings = np.zeros((num_nodes, 128))
        for i, gene in enumerate(valid_genes):
            idx = gene_mapping[gene]
            # In thông tin về chỉ số của embedding và vị trí trong gene_mapping
            print(f"Embedding {i} từ file common_genes_embedding_128.npy (gene {gene}) sẽ được thay thế vào vị trí {idx} trong gene_mapping.")
            filtered_embeddings[idx] = precomputed_embeddings[i]  # Chèn embedding mới cho các gene hợp lệ

        x = filtered_embeddings  # Thay thế embedding gốc bằng embedding mới của 407 gene

        print(f"Kích thước của x: {x.shape}")

    elif attr == 'precomputed_aug':
    # Load danh sách gene và embedding từ file mới
        #Jurkat
        gene_list_407 = pd.read_csv("../data/TransEdata/Jurkat/common_genes_Jurkat_MVGCNiSL_embeding.csv")  # 407 gene trùng
        precomputed_embeddings = np.load("../data/TransEdata/Jurkat/common_genes_embedding_128_sorted.npy")  # embedding transE của 407 gene từ bài cũ
        ...
        # Áp dụng tăng cường dữ liệu trên embedding
        aug_ratio = 0.4 # Tỉ lệ tăng cường dữ liệu
        augmented_embeddings = augment_embeddings(precomputed_embeddings, aug_ratio)

        # Tạo một ma trận embedding mới chỉ với các gene hợp lệ
        filtered_embeddings = np.zeros((num_nodes, 128))  # Placeholder cho tất cả các gene
        for i, gene in enumerate(valid_genes):
            idx = gene_mapping[gene]
            print(f"Embedding {i} từ file common_genes_embedding_128.npy (gene {gene}) sẽ được thay thế vào vị trí {idx} trong gene_mapping.")
            filtered_embeddings[idx] = augmented_embeddings[i]  # Chèn embedding đã được tăng cường cho các gene hợp lệ
        ...
        # print("Graph data overall size:", graph_data_overall.shape)
        # print(graph_data_overall.head())
        # print(f"Number of nodes in graph: {len(set(graph_data_overall['gene1']).union(set(graph_data_overall['gene2'])))}")
        # print(f"Number of edges in graph: {len(graph_data_overall)}")
        ...
        def train():
            ...
            for pos_rw, neg_rw in loader:
                ...
                # print(f"Batch loss: {loss.item()}")  # In loss của từng batch
                loss.backward()
                optimizer.step()
...
def generate_torch_geo_data(cell_name, CCLE_feats_flag, CCLE_hidden_dim, node2vec_feats_flag, threshold, graph_input, attr, split_method, predict_novel_flag, training_percent):
    ...
    #  # **In ra gene_mapping và dữ liệu sau khi mapping**
    # print("Gene Mapping:")
    # print(gene_mapping)  # Kiểm tra gene mapping

    # print("Graph Data After Mapping (First Graph):")
    # print(graph_data_list[0].head())  # Kiểm tra graph sau khi mapping gene name sang node ID
